...
def upd_ratings(r):
    ...
    if items:
        old_st = r.get("statements") or []
        seen = set((x.get("d",""), (x.get("txt","") or "")[:60].lower()) for x in old_st)
        fresh = [x for x in items
                 if (x.get("d",""), (x.get("txt","") or "")[:60].lower()) not in seen]
        if fresh:
            r["statements"] = (fresh + old_st)[:48]  # копим ленту, дубли отсекаем
            changed.append("+%d выск." % len(fresh))
